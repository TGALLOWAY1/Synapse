...
- roles: 3–6 roles — { role, allowed, restricted?, dataVisibility?, notes? }. Describe **business capabilities a user has inside the product**, the way a product manager would — not how the software is built or secured. allowed = 5–15 concise capability-based actions ("Create workouts", "Invite team members", "View analytics", "Approve requests"). restricted = OPTIONAL and small (3–10), included ONLY when a limit conveys meaningful product behavior ("Cannot modify coach-created programs"); omit it entirely otherwise — never pad with obvious/exhaustive negatives. Every item must answer "is this something a user can do inside the product?". NEVER include backend/infrastructure/database/OS/networking/security-implementation details (SSL/TLS, JWT/OAuth, SQLite/Postgres/Redis, caches, migrations, telemetry, encryption keys, API endpoints/timeouts, rate limiting, feature flags, sandboxes, Kubernetes/Docker, diagnostic endpoints, server config) — those belong in architecture/security docs.
