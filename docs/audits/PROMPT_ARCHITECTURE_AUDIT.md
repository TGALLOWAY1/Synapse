...
| R8 | **De-duplicate the per-run preamble cost** (B14): once R7 lands, the ~900-token preamble sits in `systemInstruction` where it belongs; evaluate trimming `RUBRIC_DEFINITION` to the dimensions each section is actually judged on, and stop re-embedding idea+restriction+preflight 8× by moving the restriction/clarification blocks into the shared system half | Medium (cost/latency) | M | Medium — measure output quality on fixtures before/after |
