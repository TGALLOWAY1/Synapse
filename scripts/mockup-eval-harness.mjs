...
// user-agent default of 300x150 when the inlined Tailwind sheet misses
// an exotic utility.
...
// Load the precompiled Tailwind sheet that the runtime iframe inlines. Keeps
// the screenshot harness and the live preview in lockstep without depending
// on the Tailwind Play CDN (which was the source of "Tailwind styles did
// not apply" reports). The file is generated by
// `npm run mockup-css:build`.
const mockupTailwindCssPath = path.resolve(__dirname, '..', 'src/styles/mockup-tailwind.generated.css');
let mockupTailwindCss = '';
try {
  mockupTailwindCss = await fs.readFile(mockupTailwindCssPath, 'utf8');
} catch (e) {
  console.warn(`[harness] missing ${mockupTailwindCssPath}; run \`npm run mockup-css:build\` first. ${e.message}`);
}
