...
| 🗂️ | **One PRD, two focused views** | The PRD stays a single canonical artifact but reads through two coordinated tabs — **Overview** (the product brief: problem, users, value, principles, goals, scope) and **Features** (feature systems → individual features with stable IDs, a simple **Confirm feature** action, filters, and explicit traceability). Decision feedback — assumptions, decision log, deferred scope, and risks — lives in the **Decision Center** (Challenge stage), so there's one place to weigh in and "Select text to edit" stays about the PRD prose. Both views share one version, finalization state, history, and freshness; the default export is still one complete PRD (including a Decisions part). | Stable product context stays separate from fast-changing feature detail, and decisions have a single home — without fragmenting the spec into disconnected documents. |
| 🧭 | **Uncertainty-first planning & Decision Center** | Every PRD is a **working plan** with categorical readiness, the most valuable next step, and visible assumptions or decisions. A fresh draft opens with a calm **"Your draft is ready"** card whose one action — **Sharpen my plan** — walks you through Synapse's drafting assumptions as quick plain-language questions (*Sounds right / Not quite — correct it / Not sure yet*); caution styling is reserved for genuine conflicts and changed context. Each open decision arrives with **2-3 concrete suggested approaches** — tradeoffs stated, one clearly badged as Synapse's recommendation (never preselected) — plus an **Other** path for an answer in your own words. Consequential verdicts open a version-bound alignment review where each precise plan change can be accepted, edited, rejected, or deferred. Meaning-changing PRD edits enter the same reasoning history, while copy edits stay quiet. | Progress means reducing consequential uncertainty—not producing more documents. Users can make a decision, preserve its reasoning, and bring the working plan back into coherence without silent rewriting or dependency hunting. |
| 🎨 | **One aligned PRD → every asset** | Once the plan is committed, Synapse can generate the downstream build foundation — screen inventory, data model, user flows, design system, consolidated implementation plan, mockups, and annotated SVGs — in parallel. Early outputs remain available for exploration without being presented as build-ready. | A shared planning foundation replaces hand-copying the specification into a stack of disconnected documents. |
...
| 🕓 | **Everything is versioned** | Every regenerate, edit, branch-merge, and restore appends a new version with a section-aware diff (consecutive assumption/feature confirmations coalesce into a single version instead of one per click). | Non-destructive history — nothing is ever overwritten or lost. |
...
| 🔌 | **Dependency graph & explainable alignment** | Artifacts carry source references to their inputs. Project Map and artifact views distinguish **Aligned**, **Review recommended**, and **Update required**, explain why an output may be affected, state whether the evidence is definite or possible, and preserve its usefulness for exploration. Exports carry the same alignment context. Regeneration remains an explicit later action. | The workspace communicates consequences honestly: a version mismatch is not automatically called invalid, while proven and possible impact remain actionable before build handoff. |
