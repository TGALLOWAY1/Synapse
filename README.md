...
| 🧭 | **Planning intelligence & Decision Center** | PRD assumptions and specialist-review findings become durable planning records with append-only user decisions, source evidence, recommendations, tradeoffs, and history. A decision change is previewed against a specific PRD version before an explicit **Apply to plan** creates a new version; generated assets are never silently rewritten. | Important choices stay visible, reviewable, and traceable—and users can understand the downstream consequences before implementation begins. |
