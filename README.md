...
| 🧾 | **Screen contracts & review workflow** | Every generated screen is a reviewable **contract**: explicit PRD feature traceability (labeled *explicit / estimated / missing* — never claimed as semantically verified), structured states with triggers and recovery behavior, acceptance criteria, developer handoff (route, components, state variables, events, accessibility notes), risks with proposed handling, and **per-state mockup-variant tracking** from artifact metadata (not image analysis). A readiness rollup flags gaps — stale feature refs, states without behavior, decisions without branch outcomes, recommended variants without mockups — and a user-set review status always overrides the estimate while the warnings stay visible. When you're ready, an **implementation-handoff export** bundles the accepted screens — route, components, state, events, data dependencies (correlated read-only against the Data Model), acceptance criteria, QA checklist, build tasks, mockup references, and related Implementation Plan items — into a copy/downloadable **Markdown or JSON** package with a final preflight and honest caveats (label-based correlation, no embedded images, absent artifacts noted as context not defects). Older projects fall back to derived values, clearly labeled. | The Screens page becomes the bridge from PRD to implementation — what to build, what it must satisfy, and what's still missing — exportable for a coding agent, without false precision. |
