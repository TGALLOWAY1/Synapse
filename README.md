...
| 🗂️ | **One PRD, three focused views** | The PRD stays a single canonical artifact but reads through three coordinated tabs — **Overview** (the product brief: problem, users, value, principles, goals, scope), **Features** (feature systems → individual features with stable IDs, a simple **Confirm feature** action, filters, and explicit traceability), and **Decisions** (Needs Input → Assumptions to Validate → Decision Log → Deferred & Risks, with a **Confirm all** shortcut for the remaining assumptions). All three share one version, finalization state, history, and freshness; the default export is still one complete PRD in three parts. | Stable product context stays separate from fast-changing feature detail — without fragmenting the spec into disconnected documents. |
