...
| 🗂️ | **One PRD, three focused views** | The PRD stays a single canonical artifact but reads through three coordinated tabs — **Overview** (the product brief: problem, users, value, principles, goals, scope), **Features** (feature systems → individual features with stable IDs, a simple **Confirm feature** action, filters, and explicit traceability), and **Decisions** (Needs Input → Assumptions to Validate → Decision Log → Deferred & Risks, with a **Confirm all** shortcut for the remaining assumptions). All three share one version, commitment state, history, and freshness; the default export is still one complete PRD in three parts. | Stable product context stays separate from fast-changing feature detail — without fragmenting the spec into disconnected documents. |
| 🧭 | **Uncertainty-first planning & Decision Center** | The workspace moves through a **Plan → Challenge → Build** progression (plus History): every PRD is a **working plan** with categorical readiness, the most valuable next step, and visible assumptions or decisions; the Challenge stage runs adversarial review before you commit; and the Build stage reads *Explore* until the plan is committed. Consequential verdicts open a version-bound alignment review where each precise plan change can be accepted, edited, rejected, or deferred. Meaning-changing PRD edits enter the same reasoning history, while copy edits stay quiet. | Progress means reducing consequential uncertainty—not producing more documents. Users can make a decision, preserve its reasoning, and bring the working plan back into coherence without silent rewriting or dependency hunting. |
...
Commit the plan → screen inventory, data model, flows, design system, implementation plan + multi-fidelity mockups (generated *or* your own uploads) + annotated SVGs generate **in parallel** from one source of truth.
